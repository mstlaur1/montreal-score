...
const PROJECT_DIR = process.env.PROJECT_DIR || "/home/mikhailst-laurent/projects/brule-ai/montreal-score";
...
function runScript(bin: string, script: string, args: string[]): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolve, reject) => {
    execFile(bin, [script, ...args], { cwd: PROJECT_DIR, timeout: 300_000 }, (err, stdout, stderr) => {
      if (err) {
...
export async function POST(req: NextRequest) {
  ...
  for (const ds of toRun) {
    ...
    try {
      const { stdout } = await runScript(TSX, script, args);
  ...
  if (dataset === "all" || dataset === "permits") {
    try {
      await runScript(NODE, "scripts/build-fts.js", []);
      results["fts"] = { ok: true };
    } catch (err) {
